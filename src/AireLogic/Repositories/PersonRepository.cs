...
namespace AireLogic.Repositories
{
    public class PersonRepository : IPersonRepository
    {
        ...
        public async Task<int> AddPerson(Person newPerson, CancellationToken token)
        {
            ...
            var result = await Task.Run(() => _personStore.TryAdd(newPerson.Uuid, newPerson));
        ...
        {
            var result = await Task.Run(() => _personStore.TryRemove(id, out var _));
        ...
        {
            var result = await Task.Run(() => { return _personStore.Values.ToArray(); });
        ...
        {
            var result = await Task.Run(() => { return _personStore.TryGetValue(id, out var person) ? person : null; });
        ...
        {
            var original = await GetPersonSingle(person.Uuid, token);

            var result = await Task.Run(() => _personStore.TryUpdate(person.Uuid, person, original));
